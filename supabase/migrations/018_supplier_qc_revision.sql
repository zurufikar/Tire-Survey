-- Migration 018: Supplier QC revision handling.
-- Allows the survey owner to edit a QC_REVISION survey, re-check its plate,
-- update Section 1-3 data, replace/add/delete photos, and resubmit without
-- generating a new Serial Number.

begin;

-- Supplier can edit only DRAFT or QC_REVISION surveys.
drop policy if exists "surveys_supplier_update" on public.surveys;
...
using (
  ...
  and status in ('DRAFT','QC_REVISION')
)
with check (
  supplier_id = auth.uid()
  and public.current_user_is_active()
...
-- Axle configuration follows the editable survey state.
drop policy if exists "survey_axles_supplier_insert" on public.survey_axle_configs;
...
with check (
  exists (
    ...
    where s.id = survey_axle_configs.survey_id
      ...
      and s.status in ('DRAFT','QC_REVISION')
      and public.current_user_is_active()
      and public.current_app_role() = 'supplier'
  )
);

drop policy if exists "survey_axles_supplier_update" on public.survey_axle_configs;
...
-- Tire positions may be created/updated while correcting a revision.
drop policy if exists "survey_tires_supplier_insert" on public.survey_tires;
...
drop policy if exists "survey_tires_supplier_update" on public.survey_tires;
...
-- Vehicle/tire metadata can be modified only for the owner in DRAFT/QC_REVISION.
...
-- Storage permissions must match the metadata policies above.
...
-- Rebuild submit RPC so QC_REVISION returns to QC without generating a new SN.
...
begin
  ...
  if not found then
    return jsonb_build_object('ok', false, 'errors', jsonb_build_array('Survey tidak ditemukan atau bukan milik Supplier ini.'));
  end if;

  if v_survey.status = 'QC_REVISION'::public.survey_status then
    v_is_revision := true;
  elsif v_survey.status is distinct from 'DRAFT'::public.survey_status then
    return jsonb_build_object('ok', false, 'errors', jsonb_build_array('Survey ini tidak dapat disubmit pada status saat ini.'));
  end if;
  ...
  if v_is_revision then
    v_serial := v_survey.serial_number;
    if nullif(trim(v_serial), '') is null then
      return jsonb_build_object('ok', false, 'errors', jsonb_build_array('Serial Number survey revisi tidak ditemukan.'));
    end if;
  else
  ...
  values (
    p_survey_id,
    auth.uid(),
    case when v_is_revision then 'SUPPLIER_RESUBMITTED_AFTER_QC_REVISION' else 'SUPPLIER_SUBMITTED' end,
    jsonb_build_object('status', case when v_is_revision then 'QC_REVISION' else 'DRAFT' end, 'serial_number', v_survey.serial_number),
  ...
  return jsonb_build_object(
    ...
    'status', 'SUBMITTED',
    'revision_resubmission', v_is_revision
  );
end;
$$;
